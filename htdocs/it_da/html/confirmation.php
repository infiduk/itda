...
<table class="table" bgcolor="#DDDDDD" height="50">
	...
	<?
			if($type == 0) // 학력
				echo("<td><br><h3>학력 인증하기</h3></td>");
			if($type == 1) // 경력
				echo("<td><br><h3>경력 인증하기</h3></td>");
			if($type == 2) // 자격증
				echo("<td><br><h3>자격증 인증하기</h3></td>");
			if($type == 3) // 수상내역
				echo("<td><br><h3>수상내역 인증하기</h3></td>");
			if($type == 4) // 사회활동
				echo("<td><br><h3>공모전/교내활동/대외활동/봉사활동 인증하기</h3></td>");
	?>
...
<?
			if($type == 0){// 학력
?>
<form name="form2" method="post" action="confirmation_insert.php"  enctype="multipart/form-data">
<input type="hidden" name="type" value="<?=$type?>">
<table class="table  table-bordered  table-sm  mymargin5" width="300" align="center">
    <tr class="mycolor2">
        <td> 학교명<BR></td>
		<td>
			<input type="hidden" name="com_no" value="" required="" readonly>
			<input type="text" name="com_name" value="" size="30" required="" readonly> &nbsp&nbsp
...
<table class="table" width="300" border="0" cellspacing="0" cellpadding="7">
	<tr> 
		<td align="center">
			<input class="btn btn-default" type="submit" value="요청하기"> &nbsp;&nbsp
